...
const MobileQRScanner = () => {
  ...
  const processQRData = async (qrData) => {
    try {
      ...
      // Parse QR data - it should contain student ID
      let studentId

      try {
        // Try to parse as JSON first (as generated by backend)
      ...
      if (studentsResponse.data.students && studentsResponse.data.students.length > 0) {
        const foundStudent = studentsResponse.data.students.find((student) => student.studentId === studentId)

        if (foundStudent) {
          setScannedStudent(foundStudent)
          setSelectedSession(labSessions[0])
          stopCamera() // Stop scanning once student is found
          setError("")
        } else {
      ...
      if (videoRef.current) {
        ...
        scanningIntervalRef.current = setInterval(() => {
          if (videoRef.current && canvasRef.current && videoRef.current.readyState === 4) {
            ...
            scanQRCode(imageData).then((qrData) => {
              if (qrData) {
  ...
  const handleManualInput = async () => {
    ...
    await processQRData(studentId.trim())
  }
      ...
      <ScannerCard>
        ...
        {!isScanning ? (
          ...
          <div>
            ...
            <p style={{ fontSize: "12px", color: "#666", marginTop: "10px" }}>
              Position the QR code within the blue frame. Scanning automatically...
            </p>
          </div>
        )}
